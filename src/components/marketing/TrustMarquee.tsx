...
export function TrustMarquee({
  ...
  return (
    <section aria-label="Partner logos" className={cn("bg-page py-10 md:py-12", className)}>
      <Container>
        ...
        <ul className="grid grid-cols-3 items-center justify-items-center gap-x-8 gap-y-8 md:grid-cols-6 md:gap-x-12">
          {PARTNERS.map((partner, i) => (
            <li key={partner.name + i} className="flex h-9 items-center justify-center" data-placeholder={partner.isPlaceholder}>
              <Image
                src={partner.logo}
                alt={partner.name}
                width={partner.width}
